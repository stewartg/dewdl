...
def test_udl_query_factor():
    eo_str = UDLQuery(UDLBaseDataType.EO_OBSERVATION).to_string()
...
def test_before():
    ...
    eo_str = UDLQuery(UDLBaseDataType.EO_OBSERVATION).before(epoch).to_string()
    unstub()
    assert eo_str == "test_base?obTime=%3C2015-09-16T00:00:00.000000Z&maxResults=10000"


def test_after():
    when(UDLQuery)._build_base_url().thenReturn("test_base")
    epoch = datetime(2015, 9, 16)
    eo_str = UDLQuery(UDLBaseDataType.EO_OBSERVATION).after(epoch).to_string()
    unstub()
...
def test_between():
    ...
    eo_str = UDLQuery(UDLBaseDataType.EO_OBSERVATION).between(start, end).to_string()
    unstub()
...
def test_with_uuid():
    when(UDLQuery)._build_base_url().thenReturn("test_base")
    eo_str = UDLQuery(UDLBaseDataType.EO_OBSERVATION).with_uuid("1234").to_string()
    unstub()
...
def test_from_source():
    when(UDLQuery)._build_base_url().thenReturn("test_base")
    eo_str = UDLQuery(UDLBaseDataType.EO_OBSERVATION).from_source("test_source").to_string()
    unstub()
...
def test_max_results():
    when(UDLQuery)._build_base_url().thenReturn("test_base")
    eo_str = UDLQuery(UDLBaseDataType.EO_OBSERVATION).max_results(5).to_string()
    unstub()
...
def test_with_descriptor():
    when(UDLQuery)._build_base_url().thenReturn("test_base")
    eo_str = UDLQuery(UDLBaseDataType.EO_OBSERVATION).with_descriptor("test_descriptor").to_string()
    unstub()
